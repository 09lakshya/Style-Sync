...
export function AuthInput({ label, error, id, name, className = '', ...props }: AuthInputProps) {
  const defaultName = name || (label.toLowerCase().includes('email') ? 'email' : label.toLowerCase().includes('name') ? 'name' : undefined)
  const inputId = id || defaultName || `auth-input-${label.toLowerCase().replace(/\s+/g, '-')}`
